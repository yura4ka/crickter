...
// QueryLikedPosts chains the current query on the "likedPosts" edge.
...
// the "likedPosts" edge. The optional arguments are used to configure the query builder of the edge.
...
func (uq *UserQuery) loadLikedPosts(ctx context.Context, query *PostQuery, nodes []*User, init func(*User), assign func(*User, *Post)) error {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "likedPosts" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
