...
type UserQuery struct {
	...
	withPostReactions    *PostReactionQuery
	withCommentReactions *CommentReactionQuery
...
// QueryPostReactions chains the current query on the "postReactions" edge.
func (uq *UserQuery) QueryPostReactions() *PostReactionQuery {
	query := (&PostReactionClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryCommentReactions chains the current query on the "commentReactions" edge.
func (uq *UserQuery) QueryCommentReactions() *CommentReactionQuery {
	query := (&CommentReactionClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withPostReactions:    uq.withPostReactions.Clone(),
		withCommentReactions: uq.withCommentReactions.Clone(),
...
// WithPostReactions tells the query-builder to eager-load the nodes that are connected to
// the "postReactions" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithPostReactions(opts ...func(*PostReactionQuery)) *UserQuery {
	query := (&PostReactionClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withPostReactions = query
	return uq
}

// WithCommentReactions tells the query-builder to eager-load the nodes that are connected to
// the "commentReactions" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithCommentReactions(opts ...func(*CommentReactionQuery)) *UserQuery {
	query := (&CommentReactionClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withCommentReactions = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		loadedTypes = [5]bool{
			...
			uq.withPostReactions != nil,
			uq.withCommentReactions != nil,
		}
	)
	...
	if query := uq.withPostReactions; query != nil {
		if err := uq.loadPostReactions(ctx, query, nodes,
			func(n *User) { n.Edges.PostReactions = []*PostReaction{} },
			func(n *User, e *PostReaction) { n.Edges.PostReactions = append(n.Edges.PostReactions, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withCommentReactions; query != nil {
		if err := uq.loadCommentReactions(ctx, query, nodes,
			func(n *User) { n.Edges.CommentReactions = []*CommentReaction{} },
			func(n *User, e *CommentReaction) { n.Edges.CommentReactions = append(n.Edges.CommentReactions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadPostReactions(ctx context.Context, query *PostReactionQuery, nodes []*User, init func(*User), assign func(*User, *PostReaction)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.PostReaction(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.PostReactionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_post_reactions
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_post_reactions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_post_reactions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (uq *UserQuery) loadCommentReactions(ctx context.Context, query *CommentReactionQuery, nodes []*User, init func(*User), assign func(*User, *CommentReaction)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.CommentReaction(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.CommentReactionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_comment_reactions
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_comment_reactions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_comment_reactions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
